...
\begin{document}
	...
	\begin{frame}{Time harmonic solutions of the wave equation}
		...
		\begin{equation}
			p(\cdot,t) = \Re\left\{ P(\cdot) e^{-i\omega t} \right\},
		\end{equation}
		\visible<2->{where $P:\Omega\to \mathbb{C}$ and $\omega$ is the \textbf{angular frequency}.}
		$\newline$
		\visible<3->{Substituting this ansatz into the wave equation, we obtain the \textbf{Helmholtz equation}
		\begin{equation}
			\begin{aligned}
			...
			\end{aligned}
		\end{equation}}
		\visible<4->{where $k = \omega/c$ is the \textbf{wave number} and $F = \Re\left\{f(\cdot,t)e^{-i\omega t}\right\}$.}
	\end{frame}
	\begin{frame}{The weak formulation}
		...
		\begin{equation}
			\begin{aligned}
				(\nabla P, \nabla Q)_{L^2} - k^2 (P,Q)_{L^2} &= \langle F,Q \rangle_{L^2} \quad \forall Q \in H^1_0(\Omega),
			\end{aligned}
		\end{equation}
	...
	\begin{frame}{Hilbert basis}
		...
		\begin{itemize}
			...
			\item<3->[\oxarrow] We can expand any $\Phi\in H^1_0(\Omega)$ as $P = \sum_{i\in\mathbb{N}} \Phi^{(i)}( P,\Phi^{(i)})_{H^1}$.
			\item<4->[\oxarrow]u We will adopt the convection that the $\norm{P}_{H^1}$ is unitary.
		\end{itemize}
	...
	\vspace{1.2cm}
	$\lVert \Phi^{(i)} \lVert_{H^1} = 1$ implies $\lVert \Phi^{(i)} \lVert_{L^2} = (1+\lambda^{(i)})^{-1/2}$. \\
	\vspace{-0.5cm}
	\visible<2->{
	\begin{align*}
		...
		&=\sum_{i \in \mathbb{N}} \left( \frac{\lambda^{(i)} - k^2}{1-\lambda^{(i)}} \right) P^{(i)} Q^{(i)}
	\end{align*}
	}
	\end{frame}
	...
	\begin{definition}[weak T-coercivity]
		A linear operator $A \in L(X)$ is called \textbf{weakly T-coercive} if there exists a bijective operator $T \in L(X)$ and $K \in L(X)$ compact s.t. $AT + K$ is coercive. 
	\end{definition}
	\begin{itemize}
	\item<2->[\oxarrow] $A$ is T-coercive if $AT = B$, $B$ bijective
	\item<3->[\oxarrow] $A$ is weakly T-coercive if $AT = B + K$, $B$ bijective and $K$ compact
	\end{itemize}
	...
	\begin{frame}{Schatz argument}
		...
		\begin{equation*}
			C_\mathcal{I} h \norm{Z}_{H^2(\Omega)} \leq \psi(X_h) \leq \left(\frac{\mathcal{G}}{2k^2M^2}\right)^{\frac{1}{2}}.
		\end{equation*}
		\vspace{-0.5cm}
		\visible<2->{
	...
	\\
	\visible<3->{
	\textbf{Note:} it is usually enough to show that 
	\vspace{-0.2cm}
	\begin{equation*}
